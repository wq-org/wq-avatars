...
- `default_avatars.json` is the typed source of truth (id, name, image path).
- `src/` provides type definitions and helpers:
  - `src/types.ts` (`DefaultAvatar`)
  - `src/data.ts` re-exports `default_avatars.json` as a typed `const`
...
```ts
import { getAvatar, getAllAvatars, defaultAvatars, type DefaultAvatar } from "wq-avatars";
